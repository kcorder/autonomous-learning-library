...
class GeneralizedAdvantageBuffer(Schedulable):
    def __init__(
        ...
        self._compute_batch_size = compute_batch_size
        self._states = []
        self._actions = []
        self._rewards = []
    ...
    def advantages(self, states):
        ...
        _values = states.flatten().batch_execute(self._compute_batch_size, lambda s: self.v.target(self.features.target(s))).view(states.shape)
